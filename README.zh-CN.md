...
当前公开 Release 为面向 Apple Silicon、运行 macOS 13 或更高版本的 **Quantum Leap 1.3.0**。
...
3. 确认参与测速的 macOS 本机或远端设备均可执行 `iperf3 --version`。
...
Windows NSIS、Linux AppImage 与 Linux DEB 目前由 CI 生成测试制品，尚未作为正式签名 Release 发布。Windows 测试构建可能触发 SmartScreen 提示。
...
Linux/Windows 打包前分别运行 `npm run sidecar:linux` 或 `npm run sidecar:windows`。Windows sidecar 构建需要 Cygwin 的 GCC、make、autoconf、automake 和 curl；生成的二进制与 DLL 位于被 Git 忽略的 `src-tauri/binaries/` 目录。

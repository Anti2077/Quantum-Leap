...
const ENGLISH_REPLACEMENTS: &[(&str, &str)] = &[
    ...
    ("IPERF3_PATH 指向的文件不可执行", "The file specified by IPERF3_PATH is not executable"),
    ("内置 iperf3 不可用，应用安装可能不完整", "The bundled iperf3 executable is unavailable. The application installation may be incomplete."),
    ("无法打开系统凭据存储", "Unable to open the system credential store"),
    ("保存密码到系统凭据存储失败", "Unable to save the password to the system credential store"),
    ("从系统凭据存储读取密码失败", "Unable to read the password from the system credential store"),

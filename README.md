...
- 本机与远端服务器建议安装 `iperf3` 3.12 或更高版本；3.17+ 使用 JSON 流实时模式，旧版本自动使用兼容文本模式
...
`iperf3 --json-stream` 可以逐间隔输出结构化事件，适合驱动实时曲线；支持该参数的版本会优先使用 JSON 流。较旧的 `iperf3` 会自动回退到 `-i 0.5` 文本间隔输出，仍可实时测速，但部分质量指标可能不可用。`iperf3 -J` 只在测试结束后输出一次 JSON，因此不作为实时模式的首选。
